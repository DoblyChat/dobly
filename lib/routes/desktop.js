    ...
    self.get = function(req, res) {
                            ...
                            function(callback){
                                ...
                                function loadItems(collaborationObject, callback){
                                    if (collaborationObject.type === 'C') {
                                        ...
                                        Task.readTasks(collaborationObject._id, function(err, tasks){
                                            collaborationObject.items = tasks;
                                            callback(err);
                                        });
                                    }
                                }
        ...
        function(err, results) {
            function addUnread(collaborationObject){
                ...
                results.markers.forEach(function(marker){
                    if(marker.collaborationObjectId.equals(collaborationObject._id)){
                        ...
                        if(results.desktop.collaborationObjects.indexOf(collaborationObject._id) < 0){
                            results.desktop.collaborationObjects.push(collaborationObject._id);
                        }
            ...
            if(err){
                ...
                if(results.desktop.isModified('collaborationObjects')){
