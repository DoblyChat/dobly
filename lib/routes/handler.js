module.exports = (function (){
    ...
    self.groupSignUp = function(req, res) {
        var flash = req.flash(flashKey);
        res.render('security/group-signup', { title: 'Sign up - ' + title, showFlash: shouldShow(flash), info: flash });
    }
    ...
    self.renderDesktop = function(req, res) {
        async.parallel({
            ...
            users: function(callback){
                User.find({ groupId: req.user.groupId }, '_id name', { lean: true }, callback);
            }
        },
            ...
            function addCreatorName(conversation, users){
                results.users.forEach(function(user){
                    if(user._id.equals(conversation.createdById)){
                        conversation.createdBy = user.name;
                        return;
                    }
                });
            }
