...
exports.config = function(io, sessionStore){
    ...
    io.sockets.on('connection', function (socket) {
      ...
      socket.on('subscribe_to_collaboration_objects', function(collaborationObjects){
        userIo.subscribeToCollaborationObjects(socket, collaborationObjects);
      });

      socket.on('unsubscribe_to_collaboration_object', function(collaborationObjectId){
        userIo.unsubscribeToCollaborationObject(socket, collaborationObjectId);
      });

      socket.whenUser('add_to_desktop', desktopIo.addCollaborationObject);
      socket.whenUser('remove_from_desktop', desktopIo.removeCollaborationObject);
      ...
      socket.whenUser('remove_task', function(data){
...
function broadcastToCollaborationObjectMembers(event, collaborationObjectId, data){
  console.log('in conversation', this.in('c-' + collaborationObjectId).roomClients);
