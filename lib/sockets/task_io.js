...
module.exports = (function (){
    ...
    self.assign = function(socket, data){
        Task.update({ _id: data.id }, { assignedToId: data.assignedToId }, function(err){
            if(err){
                log.error(err, 'Error assigning task.');
            }else{
                socket.broadcastToCollaborationObjectMembers('task_assigned', data.collaborationObjectId, data);
            }
        });
    };
